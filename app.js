...
if (typeof document !== 'undefined') {
  ...
  function consentHtml(key, vm) {
    ...
    return `
      ...
      <button type="button" class="btn-primary" data-action="send-consent">Text consent to ${esc(ZELLE_DISPLAY)}</button>
      ${s.consentFallback ? `<div class="fallback">Messages didn't open? Copy the line above and text it to <strong>${esc(ZELLE_DISPLAY)}</strong>.</div>` : ''}
  ...
  app.addEventListener('click', (e) => {
    ...
    if (action === 'send-consent') {
      ...
      const sep = isIOS ? '&' : '?';
      try { window.location.href = `sms:+1${ZELLE_DIGITS}${sep}body=${encodeURIComponent(vm.consentText)}`; } catch { /* sms handoff unsupported here */ }
